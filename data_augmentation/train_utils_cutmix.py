...
def train(model, runner, lr_update_func, device, epoch, epoches, meta):
    ...
    with tqdm(total=len(runner.get('train_loader')),desc=f'Train: Epoch {epoch + 1}/{epoches}',postfix=dict,mininterval=0.3) as pbar:
        for iter, batch in enumerate(runner.get('train_loader')):            
            ...
            cutmix数据增强方法
            '''
            lamuta=np.random.beta(32, 32)#出来一个概率，0.5上下浮动
            W=images.shape[2]
            H=images.shape[3]
            cut_rat=np.sqrt(1.-lamuta)
            cut_w=np.int(W*cut_rat)
            cut_h = np.int(H * cut_rat)
            cx=np.random.randint(W)#返回0.w之间的一个值
            cy=np.random.randint(H)
            bbx1 = np.clip(cx - cut_w // 2, 0, W)#截取函数
            ...
            l1 = model(images, targets=target_a, return_loss=True)
            l2 = model(images, targets=target_b, return_loss=True)
            l_cutmix = l1.get('loss') * lam + l2.get('loss') * (1 - lam)
            losses = {'loss': l_cutmix}
            print("losss",losses)
            '''
            mixup数据增强方法
            '''
            ...
            #print('get',losses.get('loss'))
            # l1=model(images_a, targets=labels_a, return_loss=True)
            # l2=model(images_b, targets=labels_b, return_loss=True)
            # l_mix = l1.get('loss')*lam+l2.get('loss')*(1-lam)
            # losses={'loss':l_mix}
