...
def accuracy_torch(pred, target, topk=(1, ), thrs=0.):
    ...
    pred_score, pred_label = pred.topk(maxk, dim=1) # 取出各行前maxk个置信度得分与索引，每行按从高到低排序
    pred_label = pred_label.t() # 转置，即把置信度按“行”从高到低排序，topk=3的话就是三行，第一行为置信度最大，每列代表不同图像
    correct = pred_label.eq(target.view(1, -1).expand_as(pred_label)) # 先将target展平，分别与每行做对比，判断是否对应相等
    for k in topk:
        res_thr = []
        for thr in thrs:
            # Only prediction values larger than thr are counted as correct
            _correct = correct & (pred_score.t() > thr) # 在判断正确的基础上继续看是否得分大于阈值
            correct_k = _correct[:k].reshape(-1).float().sum(0, keepdim=True) # 先展平成一个维度，再判断True的个数
            res_thr.append((correct_k.mul_(100. / num))) # 点乘计算准确率，已转换为100%格式，即50%，60%等
        if res_single:
            res.append(res_thr[0])
        else:
            res.append(res_thr)
...
def accuracy(pred, target, topk=1, thrs=0.):
    ...
    if isinstance(topk, int):
        ...
        return_single = False # 即多个值，top1，top5等
